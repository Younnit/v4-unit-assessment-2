...
//CODE HERE
const jsNinja = _ => 'I am a JavaScript ninja!'
...
function checker(){
    for(let i = 0; i < classes.length; i++){
        for(let key in classes[i]){
            classes[i][key] === true ? classes[i][key] = false : key
        }
    }
}
...
for(let i = 0; i < lettersToPair.length; i++){
    for(let j = 0; j < lettersToPair[i].length; j++ ){
        if(lettersToPair[i][j] === lettersToPair[i]){
            pairsArray.push(i, j)
...
//CODE HERE
  let phone1 = new Phone('iPhone', 'X', 128, 'red', false)
  
  let phone2 = new Phone('Samsung', 'Galaxy', 256, 'black', false)
  
  let phone3 = new Phone('Samsung', 'Note', 500, 'Silver', false)
